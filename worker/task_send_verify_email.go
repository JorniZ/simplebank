...
func (distributor *RedisTaskDistributor) DistributeTaskSendVerifyEmail(ctx context.Context, payload *PayloadSendVerifyEmail, opts ...asynq.Option) error {
	...
	if err != nil {
		return fmt.Errorf("failed to marshal task payload: %s", err.Error())
	}
	...
	log.Info().Str("type", task.Type()).Str("payload", string(task.Payload())).Str("queue", info.Queue).Int("max_retry", info.MaxRetry).Msg("enqueued task")
	return nil
}
